...
func start_copy(dice_face):
		...
		add_child(copy)
		copy.setup_hex_data(dice_face.hex_data)
...
func finish_drag():
	...
	if dice_slot_found and not dice_slot_found.dice_face_in_slot:
		...
		var face_data = dice_face_being_dragged.hex_data
		var hex_dict = {
			"hex_name": face_data.hex_name,
			"texture_path": face_data.texture_path,
			"cast_condition": face_data.cast_condition,
			"target_data": face_data.target_data,
			"effect_data": face_data.effect_data,
			"is_peak": face_data.is_peak,
			"is_soul": face_data.is_soul
		}
		DiceFaceChanger.update_dice_face("hex2", int(dice_face_str) - 1, (add_two(texture_path)))
		EmbeddedDiceFaces.update_dice_face("hex2", int(dice_face_str) - 1, hex_dict)
	else:
