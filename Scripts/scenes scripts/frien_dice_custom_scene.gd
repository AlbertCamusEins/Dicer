...
func _ready() -> void:
	if EmbeddedDiceFaces.is_default_dice("frien"):
		stored_faces.clear()
...
func store_faces():
	for child in face_insert_manager.get_children():
		stored_faces.append(
			...
			"character_data": child.character_data,
...
func store_face_by_slot(dice_face_being_dragged, slot_number):
	stored_faces.append({
		...
		"character_data": dice_face_being_dragged.character_data,
...
func restore_stored_faces():
	...
	for face_data in stored_faces:
		...
		dice_face.setup_character_data(face_data.character_data)
		dice_face.name = face_data.child_name
		dice_face.position = face_data.position



func load_available_characters():
	var available_characters = GameManager.current_run.inventory.get_faces("frien")
	...
	for i in range(available_characters.size()):
		...
		spawn_characters_at_position(character, pos)

func spawn_characters_at_position(character_data: CharacterData, pos: Vector2):
	var dice_face_scene = preload("res://Scenes/dice_face.tscn")
	var dice_face = dice_face_scene.instantiate()
	dice_face.setup_character_data(character_data)
	dice_face.name = character_data.character_name
	add_child(dice_face)
...
func apply_stored_player_frien_dice_faces() ->void:
	var character_faces = EmbeddedDiceFaces.get_dice_faces("frien")

	for face_index in character_faces.keys():
		var character_data = character_faces[face_index]
...
func _on_autofill_friendice_button_pressed() -> void:
	for child in face_insert_manager.get_children():
		child.queue_free()
	
	await get_tree().process_frame
	
	DiceManager.auto_fill_dice_faces("frien")
	apply_stored_player_frien_dice_faces()
