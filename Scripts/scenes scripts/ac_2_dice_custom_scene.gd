...
func _ready() -> void:
	if EmbeddedDiceFaces.is_default_dice("ac2"):
		stored_faces.clear()

	load_available_uncommon_actions()
...
func load_available_uncommon_actions():
	var available_uncommon_actions = GameManager.current_run.inventory.get_faces("uncommon_action")
	var container_pos = dice_face_container.position
	var cell_side_length = 112
	var grid_cols = 3
	var _grid_rows = 2
	
	for i in range(available_uncommon_actions.size()):
		var action = available_uncommon_actions[i]
...
func _on_autofill_uncommon_dice_button_pressed() -> void:
	for child in face_insert_manager.get_children():
		child.queue_free()
	# 等待一帧确保清除完成
	await get_tree().process_frame
	
	DiceManager.auto_fill_dice_faces("ac2")
	apply_stored_player_uncommon_ac_dice_faces()
