...
func start_copy(dice_face):
		...
		add_child(copy)
		copy.setup_character_data(dice_face.character_data)
...
func finish_drag():
	...
	if dice_slot_found and not dice_slot_found.dice_face_in_slot:
		#Dice dropped into empty dice slot
		...
		var face_data = dice_face_being_dragged.character_data
		var frien_dict = {
			"character_name": face_data.character_name,
			"texture_path": face_data.texture_path,
			"health": face_data.health
		}
		DiceFaceChanger.update_dice_face("frien", int(dice_face_str) - 1, (add_two(texture_path)))
		EmbeddedDiceFaces.update_dice_face("frien", int(dice_face_str) - 1, frien_dict)
		get_parent().store_face_by_slot(dice_face_being_dragged, dice_face_being_dragged.name)
	else:
